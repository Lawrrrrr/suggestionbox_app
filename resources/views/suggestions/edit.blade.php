...
<div class="row d-flex justify-content-center my-5">
    <div class="col-md-6 border border-secondary rounded">
        <div class="m-3">
            <h3>Edit a Suggestion</h3>
            <form action="{{ route('suggestions.update', ['id' => $suggestion->id]) }}" method="post">
                @method('PATCH')
                @csrf
                <div class="form-group">
                    <label for="">Author:</label>
                    <input type="text" name="author" id="" value="{{ $suggestion->author }}" class="form-control">
                </div>
                <div class="form-group">
                    <label for="">Content:</label>
                    <input type="text" name="content" id="" value="{{ $suggestion->content }}" class="form-control">
                </div>
